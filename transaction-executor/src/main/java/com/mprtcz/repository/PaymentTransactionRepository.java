...
import com.mprtcz.repository.data.Account;
...
public class PaymentTransactionRepository {
    ...
    public void transfer(String fromAccount, String toAccount, BigDecimal amount) {
        ...
        if (fromAccountOpt.isEmpty() || fromAccountOpt.get().balance().compareTo(amount) < 0) {
            throw new IllegalArgumentException("Insufficient funds");
        }

        int debitResult = updateBalance(fromAccount, amount.negate());
        if (debitResult == 0) {
            throw new RuntimeException("Failed to debit the amount");
        }

        int creditResult = updateBalance(toAccount, amount);
        if (creditResult == 0) {
            throw new RuntimeException("Failed to credit the amount");
        }
    }
